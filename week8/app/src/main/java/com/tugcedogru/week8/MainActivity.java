...
// We don't have permission so prompt the user
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == REQUEST_EXTERNAL_STORAGE){
            if (grantResults.length == 2 &&
                ...
                Toast.makeText(this, "External storage permission not granded", Toast.LENGTH_SHORT).show();
            }
    ...
    private Bitmap scaledBitmap(String imagePath) {
        Bitmap image = BitmapFactory.decodeFile(imagePath);
        float w = image.getWidth();
        float h = image.getHeight();
        int W = 400;
    ...
    class DownloadRunnable implements Runnable{
        ...
        public void run() {
            ...
            Bitmap bitmap = scaledBitmap(imagePath);
            runOnUiThread(new UpdateBİtmap(bitmap));
        }
    }

    class UpdateBİtmap implements Runnable{

        Bitmap bitmap;

        public UpdateBİtmap(Bitmap bitmap) {
            this.bitmap = bitmap;
        }
